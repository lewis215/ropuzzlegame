...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ANIMATION_DURATION_MS_DISAPPEAR = 400; // 消除動畫時間 (配合 CSS)
    ...
    let tiles = []; // DOM 元素引用
    let score = 0;
    let isAnimating = false; // 是否正在播放動畫
    let moveCount = INITIAL_MOVES;
    let currentComboCount = 0; // 本次操作的 Combo 數
    ...
    async function initGame() {
        ...
        if (savedGame) {
            console.log("載入已儲存的遊戲...");
            board = savedGame.board;
            score = savedGame.score;
            moveCount = savedGame.moveCount !== undefined ? savedGame.moveCount : INITIAL_MOVES;
            updateScoreDisplay();
            updateMovesDisplay();
            renderBoard(true);
        } else {
            ...
            if (!board || board.length === 0) {
                console.error("錯誤：createInitialBoard 返回無效版面！");
                isAnimating = false;
                gameBoardElement.innerHTML = '<p style="color: red;">遊戲初始化失敗！</p>';
                return;
            }
            score = 0;
            moveCount = INITIAL_MOVES;
        ...
        console.log(`初始分數: ${score}, 初始移動次數: ${moveCount}`);
    ...
    function createInitialBoard() {
        ...
        do {
            ...
            for (let y = 0; y < BOARD_HEIGHT; y++) {
                ...
                for (let x = 0; x < BOARD_WIDTH; x++) {
                    ...
                    do {
                        ...
                        generationAttempts++;
                        if (generationAttempts > maxGenerationAttempts) break;
                    } while (
                        (x >= 2 && row[x - 1] === type && row[x - 2] === type) ||
                        (y >= 2 && newBoard[y - 1]?.[x] === type && newBoard[y - 2]?.[x] === type)
                    );
                    row.push(type);
                }
                newBoard.push(row);
            ...
            attempts++;
            if (attempts > maxAttempts) { console.error("超過最大嘗試次數"); break; }
    ...
    function renderBoard(isInitial = false) {
        gameBoardElement.innerHTML = '';
        tiles = [];
        if (!board || board.length !== BOARD_HEIGHT || !board[0] || board[0].length !== BOARD_WIDTH) {
            console.error("renderBoard 錯誤：board 數據無效！", board);
            gameBoardElement.innerHTML = '<p style="color: red;">錯誤！</p>';
            return;
        }
        for (let y = 0; y < BOARD_HEIGHT; y++) {
            const rowElements = [];
            for (let x = 0; x < BOARD_WIDTH; x++) {
                ...
                let tileElement;
                if (tileType && TILE_TYPES.includes(tileType)) {
                    tileElement = createTileElement(x, y, tileType);
                } else {
                    if (tileType !== null) console.warn(`無效 tileType "${tileType}"`);
                    tileElement = createPlaceholderElement(x, y);
                }
                gameBoardElement.appendChild(tileElement);
                rowElements.push(tileElement);
            }
            tiles.push(rowElements);
        }
    }
    ...
    function createTileElement(x, y, tileType) {
        ...
        tileElement.addEventListener('mousedown', handleDragStart);
        tileElement.addEventListener('touchstart', handleDragStart, { passive: false });
        tileElement.addEventListener('touchmove', (e) => { if (isDragging) e.preventDefault(); }, { passive: false });
        return tileElement;
    }

    // --- 創建一個空的佔位符元素 ---
    function createPlaceholderElement(x, y) {
    ...
    function handleDragEnd(event) {
        ...
        const originalDraggingTileInfo = draggingTileInfo;
        isDragging = false; draggingTileInfo = null;
        ...
        if (direction) {
            ...
            if (targetX >= 0 && targetX < BOARD_WIDTH && targetY >= 0 && targetY < BOARD_HEIGHT) {
                ...
                if (targetTileElement && targetTileElement.classList.contains('tile')) {
                    processSwap(originalDraggingTileInfo, { x: targetX, y: targetY, element: targetTileElement });
    ...
    async function processSwap(tile1Info, tile2Info) {
        ...
        // Step 2: 交換數據並檢查
        const tempType = board[tile1Info.y][tile1Info.x];
        board[tile1Info.y][tile1Info.x] = board[tile2Info.y][tile2Info.x];
        board[tile2Info.y][tile2Info.x] = tempType;
        ...
        if (matches.length > 0) {
            ...
            // 更新 tiles 引用 & dataset
            const tile1Element = tile1Info.element;
            const tile2Element = tile2Info.element;
            tiles[tile1Info.y][tile1Info.x] = tile2Element;
            tiles[tile2Info.y][tile2Info.x] = tile1Element;
            tile1Element.dataset.x = tile1Info.x; tile1Element.dataset.y = tile1Info.y;
            tile2Element.dataset.x = tile2Info.x; tile2Element.dataset.y = tile2Info.y;
            renderBoard(); // 重新渲染以同步 DOM 結構

            await wait(50); // 短暫停頓看清結果

            await handleMatchesAndRefill(true); // 開始消除流程

            saveGameProgress();
            // isAnimating 會在 handleMatchesAndRefill 內部合適時機解鎖 (或保持鎖定直到結束)
            // checkGameOver(); // Check game over after the full sequence
        } else {
            // 無匹配：數據換回，播放回彈動畫
            console.log("交換無效，回彈...");
            board[tile2Info.y][tile2Info.x] = board[tile1Info.y][tile1Info.x];
            board[tile1Info.y][tile1Info.x] = tempType;

            await animateVisualSwap(tile2Info, tile1Info); // 視覺換回來

            isAnimating = false; // 回彈結束後解鎖
        }
        // 在 processSwap 結束時檢查 (無論成功失敗，只要動畫結束了)
         if (!isAnimating) checkGameOver(); // If still animating, checkGameOver will be called later
    }
    ...
    async function animateVisualSwap(tile1Info, tile2Info) {
        ...
        const boardGap = parseFloat(getComputedStyle(gameBoardElement).gap || '0px');
        const elementWidth = tile1Element.offsetWidth; const elementHeight = tile1Element.offsetHeight;
        const dx = (tile2Info.x - tile1Info.x) * (elementWidth + boardGap);
        const dy = (tile2Info.y - tile1Info.y) * (elementHeight + boardGap);

        tile1Element.style.transition = `transform ${ANIMATION_DURATION_MS_SWAP}ms ease-in-out`;
        tile2Element.style.transition = `transform ${ANIMATION_DURATION_MS_SWAP}ms ease-in-out`;
        ...
        tile1Element.style.transition = ''; tile2Element.style.transition = '';
        tile1Element.style.transform = ''; tile2Element.style.transform = '';
        tile1Element.style.zIndex = ''; tile2Element.style.zIndex = '';
    }

    // --- 無法交換時的抖動動畫 ---
     async function animateWiggle(element) {
         if (!element || isAnimating) return;
         const tileStillAnimating = isAnimating; // 記住調用時的狀態
         isAnimating = true;
         element.style.transition = 'transform 0.07s ease-in-out'; // 加快抖動
         const originalTransform = element.style.transform;
         try {
             await wait(0); // Ensure styles apply before starting animation steps
             ...
             element.style.transform = originalTransform; await wait(70);
              ...
              element.style.transition = ''; // 清除臨時 transition
              // 只有當調用此函數時 isAnimating 為 false 時才解鎖
              // 如果調用時已經在播放其他動畫，則不改變 isAnimating 狀態
              if (!tileStillAnimating) {
                 isAnimating = false;
              }
         }
     }
    ...
    async function handleMatchesAndRefill(isInitialMove = false) {
        ...
        let shouldUnlockAnimation = true; // 假設此流程結束後應該解鎖

        while (matches.length > 0 && iteration < maxIterations) {
            iteration++; isAnimating = true; // 確保鎖定
            const previousComboCount = currentComboCount; currentComboCount += matches.length; console.log(`連鎖 ${iteration} (${matches.length}組), 總 Combo: ${currentComboCount}`);
    ...
    async function dropAndRefillBoard() {
        ...
        await animateDrops(dropInfos, ANIMATION_DURATION_MS_SWAP); // 使用交換動畫時間
        const { newBoardData, newTilesInfo } = fillEmptyTiles(boardAfterDrop); board = newBoardData; renderBoard();
        await animateNewTiles(newTilesInfo, ANIMATION_DURATION_MS_SWAP); // 使用交換動畫時間
    }
    ...
    // --- 檢查遊戲是否結束 ---
    function checkGameOver() { if (moveCount <= 0 && !isAnimating) { console.log("遊戲結束！"); isAnimating = true; setTimeout(() => { alert(`遊戲結束！\n最終分數：${score}\n\n按下「確定」重新開始。`); localStorage.removeItem('roMatch3Save'); initGame(); }, 500); return true; } return false; }
    ...
    function loadGameProgress() { const savedState = localStorage.getItem('roMatch3Save'); if (savedState) { try { const parsedState = JSON.parse(savedState); if (parsedState && Array.isArray(parsedState.board) && parsedState.board.length === BOARD_HEIGHT && parsedState.board.every(row => Array.isArray(row) && row.length === BOARD_WIDTH) && typeof parsedState.score === 'number' && (parsedState.moveCount === undefined || typeof parsedState.moveCount === 'number') ) { console.log("讀取有效進度"); if (parsedState.moveCount === undefined) parsedState.moveCount = INITIAL_MOVES; return parsedState; } else { console.warn("存檔格式錯誤"); localStorage.removeItem('roMatch3Save'); } } catch (e) { console.error("讀取失敗:", e); localStorage.removeItem('roMatch3Save'); } } return null; }

    // --- 重置按鈕事件 ---
    resetButton.addEventListener('click', () => { if (isAnimating && moveCount > 0) { alert("遊戲處理中！"); return; } console.log("點擊重置"); if (confirm("確定要重新開始嗎？")) { localStorage.removeItem('roMatch3Save'); isAnimating = false; initGame(); } });
